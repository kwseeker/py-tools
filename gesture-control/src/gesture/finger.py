...
def draw_palm_points(frame, hand_landmarks, history_palm_points=[]):
    ...
    index_finger_point = hand_landmarks.landmark[index_finger_index]
    thumb_point = hand_landmarks.landmark[thumb_index]

    finger_distance = np.sqrt(np.square(thumb_point.x - index_finger_point.x)
                              + np.square(thumb_point.y - index_finger_point.y))
    if finger_distance < 0.1:
        palm_point = [int((thumb_point.x + index_finger_point.x) * frame.shape[1] // 2),
                      int((thumb_point.y + index_finger_point.y) * frame.shape[0] // 2)]
